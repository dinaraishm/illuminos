<?php
	...
	$sql = "SELECT * FROM `products` WHERE `model` ='сериал'> 0 ORDER BY `created_at` DESC";
	...
	if($data == "")
		$data = '<h3 class="text-center">Фильм отсутствует</h3>';
...
?>
	...
	<main>
		<div class="content">
<img src="asset/img/image31.png">
			<div class="head" style="margin-bottom: 10px">movies</div>
			<div class="row" style="margin-bottom: 20px">

				<select id="category" onchange="filter.filter('category', 'filter')">
					<option value disabled selected>the genre of the film</option>
					<?= $categories ?>
				</select>
			</div>
