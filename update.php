...
?>
	...
	<main>
		<div class="content">
			...
			<div class="head">Изменить фильм</div>
			<form enctype="multipart/form-data" action="controllers/update_product.php" method="POST">
				...
				<input type="text" placeholder="Оценка" name="price" value="<?= $product["price"] ?>" required>
				<input type="text" placeholder="Актеры" name="country" value="<?= $product["country"] ?>" required>
				<input type="text" placeholder="Похожие фильмы" name="year" value="<?= $product["year"] ?>" required>
				<input type="text" placeholder="Тип" name="model" value="<?= $product["model"] ?>" required>
